...
\## Checkpoint: finalize start UI and character visuals



相较于上一版提交：

\- 完成开始界面中文化，加入更详细的玩法说明。

\- 将玩法说明拆成基本操作、英雄技能、通关目标几部分，提升游戏入口界面的完整度。

\- 接入夏洛特、花木兰和普通敌人的角色贴图，原有圆形碰撞体继续负责逻辑判断。

\- 完成花木兰剑气贴图资源加载和显示调整。

\- 调整中文字体加载方式，使用微软雅黑字体解决中文显示问题。

\- 保留 1600×900 窗口和视角方案，不再继续扩大窗口。

\- 背景音乐和台词音效暂时放弃，作为后续拓展方向保留。

\- 当前版本作为大作业游戏主体完成版，后续进入仓库整理、报告撰写和 B 站视频录制阶段。
